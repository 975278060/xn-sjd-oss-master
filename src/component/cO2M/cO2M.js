...
import { Form, Button, Table, Upload } from 'antd';
      ...
      reader.onload = (e) => {
        ...
        for(let v = 0; v < data.length; v++) {
          ...
          for(let j = 0; j < fArr.length; j++) {
            if(fArr[j].amount) {
              temp[fArr[j].field] = data[v][j + 1] * 1000;
            } else {
              temp[fArr[j].field] = data[v][j + 1];
            }
            temp.code = (new Date()).valueOf();
            console.log(temp);
          }
          finalData.push(temp);
        }
  ...
  getTableBtn(options, hasSelected) {
    ...
    return (
      <div>
        ...
        {options.import
          ? <Upload {...upProps}>
            <Button {...this.getBtnProps()}>导入</Button>
        </Upload> : null}
      </div>
    );
  }
